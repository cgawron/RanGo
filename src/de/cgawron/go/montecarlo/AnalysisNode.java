...
class AnalysisNode implements Comparable<AnalysisNode>
{
	...
	public AnalysisResult evaluateByMC(double[][] territory)
	{
		AnalysisResult result = new AnalysisResult(territory);
		AnalysisNode currentNode = this; 
		
		while (true) {
			currentNode = currentNode.selectRandomMCMove();
			...
			if (currentNode.moveNo > Evaluator.MAX_MOVES) {
				Evaluator.logger.severe("exiting after " + Evaluator.MAX_MOVES + " moves: " + this);
				return null;
			}
		} 
	...
	protected AnalysisNode selectRandomMove()
	{
		if (children == null)
			return selectRandomMCMove();
		else
			return selectRandomUCTMove();
	}
	...
	public String toString() {
		return "AnalysisNode [move=" + move
				+ ", moveNo=" + moveNo + ", value=" + value + ", movingColor=" + goban.movingColor
				+ ", visits=" + visits
